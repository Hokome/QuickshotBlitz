...
namespace QuickshotBlitz.Editor
{
    ...
    public class RuleTileGenerator : EditorWindow
    {
        ...
        public List<Dictionary<Vector3Int, int>> templ_neighbors = new();
        ...
        void LoadTemplate()
        {
            ...
            foreach (var item in templateSprites)
            {
                ...
                Dictionary<Vector3Int, int> neighborRules = new();
                ...
                foreach (var neighbor in NeighborPositions)
                {
                    ...
                    if (c == Color.white)
                    {
                        neighborRules.Add(neighbor, 0);
                    }
                    else if (c == Color.green)
                    {
                        neighborRules.Add(neighbor, RuleTile.TilingRule.Neighbor.This);
                        def = false;
                    }
                    else if (c == Color.red)
                    {
                        neighborRules.Add(neighbor, RuleTile.TilingRule.Neighbor.NotThis);
                    }
                }
        ...
        public RuleTile GenerateRuleTile()
        {
            ...
            for (int i = 0; i < tileSprites.Length; i++)
            {
                ...
                rule.ApplyNeighbors(templ_neighbors[i]);
